...
export default function LoginPage() {
  ...
  const [isBackendLocked, setIsBackendLocked] = useState(false);
    ...
    if (step === 1) {
      ...
      try {
        ...
        if (res.status === 403) {
          ...
          throw new Error(data.message || `Account locked. Try again in ${countdown || "a few hours"}`);
        }
  ...
  const handleVerifyOtp = async (e) => {
    ...
    try {
      ...
      if (res.status === 403) {
        setLockUntil(data.lock_until);
        setLockedEmail(formData.email);
        throw new Error(data.message || `Verification failed. Account locked for ${countdown || "a few hours"}`);
      }
      if (!res.ok) throw new Error(data.message || "Failed to verify OTP");

      setGeneralSuccess("Verified successfully! Signing you in...");
      login({ email: formData.email, role: "user", user_id: data.user_id });
      navigate("/dashboard");
    } catch (err) {
      setGeneralError(err.message);
    } finally {
